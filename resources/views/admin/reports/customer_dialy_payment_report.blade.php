        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    ...
                    <div class="body">
                        <div class="row clearfix">
                            {!! Form::open(array('route' => 'customer_dialy_payment_report' , 'method' => 'GET')) !!}
                            {{-- <div class="col-sm-2">
                                <b>From Date</b>
                ...
                <div class="card" id="printarea" style="padding: 10px;">
                    <div class="row">
                        <div class="col-sm-12" style="text-align: center;">
                            <h3>{{ trans('app.disbursement_loan_report') }}</h3>
                        </div>
                    </div>
                    {{-- <div class="row">
                        <div class="col-sm-12 ">
                            ...
                            <div class="table table-responsive">
                                <table class="table table-bordered" style="white-space: nowrap;">
                                    ...
                                    @php
                                        ...
                                        $sale_paid_interest = 0;
                                        $sale_notpaid_interest = 0;
                                        $total_amount_usd=0;
                                        ...
                                        $sum_p_notpaid = 0;
                                        $sum_p_amount = 0;
                                        $sum_p_paid_interest = 0;
                                        $sum_p_notpaid_interest = 0;
                                        $sum_amount = 0;
                                        $sum_amounts = 0;
                                        $currencySymbol = '$';
                                    @endphp
                                    @foreach($rows as $key => $row)
                                        @php
                                            ...
                                            $sale_paid_interest = $row->payment->where('status','<>','notpaid')->sum('t_interest');
                                            $sale_notpaid_interest = $row->payment->where('status','=','notpaid')->sum('interest');
                                            $sale_interest = $row->payment->sum('interest');
                                            $total_amount_usd +=$row->total;
                                            $sum_price += $row->price;
                                            ...
                                            $sum_p_paid_interest += $sale_paid_interest;
                                            $sum_p_notpaid_interest += $sale_notpaid_interest;
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ isset($row->customer->phone)?$row->customer->phone : 'N/A' }}</td>
                                            <td>{{ ($row->interest*1) }}%</td>
                                            <td>{{ number_format($row->total,2) }} {{$currencySymbol}}<//td>
                                            <td>{{ $sum_percentage }}%</td>
                                            <td>{{ number_format($sale_interest,2) }} {{$currencySymbol}}<//td>
                                            <td>{{ number_format($sale_paid,2) }} {{$currencySymbol}}<//td>
                                            <td>{{ number_format($sale_notpaid,2) }} {{$currencySymbol}}<//td>
                                            <td>{{ number_format($sale_paid_interest,2) }} {{$currencySymbol}}<//td>
                                            <td>{{ number_format($sale_notpaid_interest,2) }} {{$currencySymbol}}<//td>
                                            <td>{{ number_format($sum_amount,2) }} {{$currencySymbol}}<//td>
                                        </tr>
                                    @endforeach
                                    <tr style="background-color: #ddd;">
                                        <th colspan="2" style="text-align: right !important;">Total:</th>
                                        <th>{{ number_format($sum_price,2) }} {{ $currencySymbol }}<//th>
                                        <th>{{ number_format($sum_deposit,2) }} {{ $currencySymbol }}<//th>
                                        <th></th>
                                        ...
                                        <th>{{ number_format($sum_p_notpaid,2) }} {{ $currencySymbol }}<//th>
                                        <th>{{ number_format($sum_p_paid_interest,2) }} {{ $currencySymbol }}<//th>
                                        <th>{{ number_format($sum_p_notpaid_interest,2) }} {{ $currencySymbol }}<//th>
                                        <th>{{ number_format($sum_amounts,2) }} {{ $currencySymbol }}<//th>
                                    </tr>
                                    </tbody>
                                </table>
                            </div>
