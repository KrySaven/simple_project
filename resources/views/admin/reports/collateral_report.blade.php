                                    ...
                                    <tbody id="body_list">
                                        ...
                                        @forelse($data as $item)
                                            ...
                                            @foreach($item as $val)
                                                ...
                                                <tr>
                                                    @if($i == 1)
                                                        <td style="text-align: center;vertical-align: middle;" rowspan="{{ count($item) }}">{{ $no++ }}</td>
                                                        <td style="text-align: center;vertical-align: middle;" rowspan="{{ count($item) }}">{{ $val['customer_name'] }}</td>
                                                    @endif
                                                    <td>{{$col_detail_no++ }}. {{ config('app.collateral_type_kh')[$val['collateral_type']??""]??"" }}</td>
                                                    <td>{{ $val['collateral_name']??"" }}</td>
                                                    <td>{{ $val['color']??"" }}</td>
                                                    <td>{{ $val['year_of_mfg']??"N/A" }}</td>
                                                    <td>{!! $val['engine_no']??"N/A" !!}</td>
                                                    <td>{{ $val['licence_type']??"N/A" }}</td>
                                                    <td>{!! $val['frame_no']??"N/A" !!}</td>
                                                    <td>{{ $val['first_date_registeration']??"N/A" }}</td>
                                                    <td>{{ ucfirst($val['status']??"") }}</td>
                                                    <td>{!! !empty(date('d-M-Y',strtotime($val['return_date']??"")))?$val['return_date']:'N/A' !!}</td>
                                                    <td>{{ $val['return_by']??"N/A" }}</td>
                                                    <td>{!! $val['description']??"N/A" !!}</td>
                                                </tr>
                                            @endforeach
                                        @empty
                                            <tr>
                                                <td class="text-center" colspan="14">{{ trans('app.data_not_found') }}</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
