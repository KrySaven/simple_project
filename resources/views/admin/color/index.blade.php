        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                 <div class="header btnAdd">
                    ...
                    @endphp
                    @if(isset($checkisadmin) || isset($UserPermision['color.create']))
                        <a href="{{ route('color.create') }}" class="btn bg-green waves-effect">
                ...
                <div class="body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                            ...
                            <tbody>
                            @forelse ($color as $key => $row)
                                <tr>
                                    <td style="text-align: center;">{{ ++$key }}</td>
                                    <td>{{$row->name}}</td>
                                    <td>
                                        <div class="box_color" style="background: {{ $row->color_code }}"></div>
                                    </td>
                                    {{-- <td>{{ $row->color_code }}</td> --}}
                                    <td>{{ date('d/m/Y',strtotime($row->created_at)) }}</td>
                                    <td>
                                        <div class="button_demo">
                                            <a href="{{route('color.edit',['id'=>$row->id])}}"><button type="button" class="btn bg-green waves-effect"><i class="material-icons" style=" top: 0px;">mode_edit</i></button></a>
                                            <a onclick="deletes(this);" urls="{{route('color.destroy',['id'=>$row->id])}}">
                        ...
                        <div class="col-md-12 pull-right">
                            {{ $color->appends(Request::get('page'))->links()}}
                        </div>
                    </div>
                </div>
            </div>
