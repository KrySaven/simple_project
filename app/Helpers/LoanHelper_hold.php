		...
		foreach ($period as $key => $dt) {
			...
			if($durationType=="daily"){
				...
				if($this->isWeekend($nextDay, $branch_id) OR $this->isPublicHoliday($nextDay, $branch_id)){
					...
					$theAdvanceFine= ($loan_amount * $interest)/100;
				...
				$addtwodays	= 172800;
		        $nextDay	= $is_weekend;
				// dd(date('d-m-Y',$is_weekend),$next_da);
				if($this->isSunday($nextDay, $branch_id) OR $this->isPublicHoliday($nextDay, $branch_id)){
					// dd($this->isSunday($nextDay, $branch_id));
					// if($this->isSunday($nextDay, $branch_id)==true){
					// 	// dd(0);
					// 	// $nextDay = $is_weekend+$addDay;

					// 	$notification = array(
					// 		'message'       => __('app.loan_is_processing'),
					// 		'alert-type'    => 'warning'
					// 	);
					// 	// return redirect()->back()->with('warning','loan_is_processing');
					// 	// return redirect()->route('loan.create')->with('warning');
					// 	// return redirect()->back();
					// 	// collect($notification)->toJson();
					// 	return response()->json([
					// 		'status' => false,
					// 		'message' => "Customer is not exist for this vendor"
					// 	], 404);
					// }
					// dd(0);
			...
			// Add Advance Fine
			$advance_fine = $this->fixedAdvinceFine($currencyType,$key,$paymentTerm,$total_loan,$total_interest,$is_SunDay,$is_sat_day,$is_publicHoliday);
